...
function add (request) { 
	maths.add( buildOptions( request ), function( error, result ){
		renderJSON( request, error, {'equals': parseFloat(result)} );
	}); 
}
...
/*function batch (request) {
	var x = request
	request.reply(request.payload).type('application/json; charset=utf-8');
}*/
...
function storeList (request) { 
	var options = {
		query: {}
	};
	// add defaults as these querystring are optional
	options.page = (request.query.page)? parseInt( request.query.page ) : 1;
	options.pageSize = (request.query.pagesize)? parseInt( request.query.pagesize ) : 20;
